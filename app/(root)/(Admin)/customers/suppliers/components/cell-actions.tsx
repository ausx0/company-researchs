...
import { SuppliersData } from "./columns";
import { useMutation, useQueryClient } from "@tanstack/react-query";
import { useDisclosure } from "@nextui-org/react";
import SupplierModalForm from "./SupplierModalForm";
import { apiDeleteSupplier } from "@/app/services/api/Customers/Suppliers";
...
export const CellAction: React.FC<cellActionProps> = ({ data }) => {
  ...
  const DeleteSupplierMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      querySupplier.invalidateQueries({
        queryKey: ["Lab-Suppliers"],
      }); // Invalidate the 'Suppliers' query
    },
  });
  const onDelete = async () => {
    // .log(`Supplier_id = ${data.ID}`);
    DeleteSupplierMutation.mutate({ Supplier_id: data.ID });
    setOpen(false);
  };
